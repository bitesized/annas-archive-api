 ...
 * whole response.
 */
async function fetchDownloads(result) {
  try {
    const resp = await fetchWithTimeout(
      `${BASE_URL}/dyn/md5/inline_info/${result.md5}`,
 ...
 */
export async function search(query, { limit = 20, includeDownloads = true } = {}) {
  let resp;
  try {
  ...
  const results = parseSearchResults(html, BASE_URL).slice(0, limit);

  if (includeDownloads && results.length) {
    await withConcurrency(results, DOWNLOAD_CONCURRENCY, fetchDownloads);
  }
 ...
 */
export async function fastDownload(md5, key) {
  const url = new URL(`${BASE_URL}/dyn/api/fast_download.json`);
